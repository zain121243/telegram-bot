...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        cmd = 'ffmpeg -i input.mp4 -i logo.png -filter_complex "[1:v]scale=300:-1[logo];[0:v][logo]overlay=(W-w)/2:H-h-2" -c:v libx264 -preset fast -crf 23 -pix_fmt yuv420p -c:a copy -y output.mp4'
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        cmd = 'ffmpeg -i input.jpg -i logo.png -filter_complex "[1:v]scale=300:-1[logo];[0:v][logo]overlay=(W-w)/2:H-h-2" -y output.jpg'
